...
- Reloads and navigation keep the user-selected tab recording. BugReceipt attempts to reinstall console and network instrumentation after each document loads; when Chrome has not granted access to a new origin, visual recording continues without new page diagnostics until the capture stops or reaches an accessible page.
...
| Active-tab permission ends on cross-origin navigation      | Console and network evidence can pause | Keep the user-approved tab video stream recording, treat reinjection failure as non-fatal, and test both accessible and inaccessible destination origins                                                                        |
